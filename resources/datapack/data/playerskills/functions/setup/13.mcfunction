...
execute at @e[tag=setup,limit=1] run setblock ~ ~-1 ~-2 minecraft:command_block[facing=down]{ auto: 0b, Command: "/execute as @p[limit=1] run function playerskills:teleport/13_to_nether" }
...
execute at @e[tag=setup,limit=1] run setblock ~-2 ~ ~ minecraft:stone_button[face=floor,facing=east]
execute at @e[tag=setup,limit=1] run setblock ~-3 ~ ~ oak_sign[rotation=13]{ Text2: '{"text":"Pass","bold":true,"color":"dark_green"}' } replace
