...
execute at @e[tag=setup,limit=1] run setblock ~ ~-1 ~-2 minecraft:command_block[facing=down]{ auto: 0b, Command: "/execute as @p[limit=1] run function playerskills:teleport/30_to_nether" }
...
execute at @e[tag=setup,limit=1] run setblock ~ ~-1 ~2 minecraft:command_block[facing=down]{ auto: 0b, Command: "/execute as @e[tag=test_30,limit=1] at @s run function playerskills:action/reset" }
...
execute at @e[tag=setup,limit=1] run setblock ~-1 ~ ~-1 oak_sign[rotation=14]{ Text1: '{"text":"Item","bold":true}', Text2: '{"text": "is wearable"}', Text3: '{"text": "curio/trinket"}' } replace
