...
case class FriendlyBuffer(private var underlying: FriendlyBuffer.Vanilla) {
  ...
  def writeBoolean(value: Boolean): FriendlyBuffer = chain(underlying.writeBoolean(value))
  ...
  def writeDouble(value: Double): FriendlyBuffer = chain(underlying.writeDouble(value))

  def readUUID(): Option[UUID] = Option(underlying.readUUID())

  def writeUUID(value: UUID): FriendlyBuffer = chain(underlying.writeUUID(value))
