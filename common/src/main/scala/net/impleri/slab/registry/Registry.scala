...
class Registry[T <: ResourceWrapper[U], U](protected val underlying: Registry.Vanilla[U], protected val f: U => T) {
  ...
  def matchingTag(key: Tag[T, U]): Seq[ResourceLocation] = {
    underlying.getTag(key.value).toScala
      .toList
...
object Registry {
  ...
  type ENTITY_TYPE = Registry[EntityType[EntityType.AnyVanilla], EntityType.AnyVanilla]
  lazy val Entities: ENTITY_TYPE = new Registry(McRegistry.ENTITY_TYPE, EntityType(_))
  ...
  type RECIPE_TYPE = Registry[RecipeType[RecipeType.AnyVanilla], RecipeType.AnyVanilla]
  lazy val RecipeTypes: RECIPE_TYPE = new Registry(McRegistry.RECIPE_TYPE, RecipeType(_))
}
