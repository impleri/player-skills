...
import net.minecraft.commands.arguments.coordinates.{Vec3Argument, Coordinates => McCoordinates}
...
object CoordinatesArgument extends ArgumentUtils {
  ...
  def getValue(name: String, context: Command.Context): Option[Coordinates] =
    wrapParser[Coordinates.Vanilla, Coordinates]("coordinates", name, Coordinates(_)) {
      Vec3Argument.getVec3(context, name)
    }
}
