...
case class ServerLevelLifecycleEvents(
  private val onLevelLoadEvent: Event[ServerLevelState] = LifecycleEvent.SERVER_LEVEL_LOAD,
  private val onLevelUnloadEvent: Event[ServerLevelState] = LifecycleEvent.SERVER_LEVEL_UNLOAD,
  private val onLevelSaveEvent: Event[ServerLevelState] = LifecycleEvent.SERVER_LEVEL_SAVE,
) {
  ...
  def onLevelLoad(handler: ServerLevelLifecycleEvents.OnLevelLifecycle): Unit = {
    onLevelLoadEvent.register(handleEvent(_, handler))
  }

  def onLevelUnload(handler: ServerLevelLifecycleEvents.OnLevelLifecycle): Unit = {
    onLevelUnloadEvent.register(handleEvent(_, handler))
  }

  def onLevelSave(handler: ServerLevelLifecycleEvents.OnLevelLifecycle): Unit = {
