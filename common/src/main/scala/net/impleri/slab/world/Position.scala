...
case class Position(override protected val underlying: BlockPos)
    extends ResourceWrapper[BlockPos] {
  override def name: Option[ResourceLocation] = None

  override val value: BlockPos = underlying

  def asCoordinates: Option[Coordinates] =
    Coordinates(underlying.getX, underlying.getY, underlying.getZ)
