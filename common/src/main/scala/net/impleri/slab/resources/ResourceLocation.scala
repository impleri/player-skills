...
case class ResourceLocation(private val underlying: McResourceLocation) {
  ...
  def getTagKey[T <: ResourceWrapper[U], U](
    registryKey: ResourceKey.Registry[U],
  ): Tag[T, U] = new Tag(
...
object ResourceLocation {
  ...
  def apply(resource: Option[McResourceLocation]): Option[ResourceLocation] =
    resource.map(r => new ResourceLocation(r))
  ...
  def apply(namespace: String, path: String): Option[ResourceLocation] =
    Try(
      ...
      case Failure(_) =>
        Logger.DEFAULT
          .error(
            s"Could not parse resource location $namespace${McResourceLocation.NAMESPACE_SEPARATOR}$path",
          )
      case _ => ()
    }.toOption
      .flatMap(apply)
  ...
  def apply(resource: String): Option[ResourceLocation] =
    Try(
      ...
      case Failure(_) =>
        Logger.DEFAULT.error(s"Could not parse resource location $resource")
      case _ => ()
    }.toOption
      .flatMap(apply)
}
