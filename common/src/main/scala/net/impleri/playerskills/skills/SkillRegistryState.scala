...
object SkillRegistryState {
  ...
  final case class Skills private[skills] (skills: List[Skill[_]]) {
    ...
    def get(key: ResourceLocation): Option[Skill[_]] =
      skills.find(_.name == key)

    def has(key: ResourceLocation): Boolean = skills.exists(_.name == key)

    def upsert(skill: Skill[_]): Skills = Skills(
  ...
  private def readOp[T](f: Skills => T): State[Skills, T] =
    State[Skills, T](s => (s, f(s)))

  val empty: Skills = Skills(List.empty)

  def resync(source: List[Skill[_]]): State[Skills, Unit] =
    State.set(Skills(source))

  def upsert(skill: Skill[_]): State[Skills, Unit] =
    State.modify(_.upsert(skill))

  def add(skill: Skill[_]): StateT[ErrorOr, Skills, Unit] =
    StateT.modifyF[ErrorOr, Skills](_.add(skill))

  def remove(key: ResourceLocation): State[Skills, Unit] =
    State.modify(_.remove(key))

  def entries(): State[Skills, List[Skill[_]]] =
    State[Skills, List[Skill[_]]](s => (s, s.entries))

  def find(key: ResourceLocation): State[Skills, Option[Skill[_]]] = readOp(
    _.get(key),
  )
