...
object RestrictionRegistryState {
  final case class Restrictions private[restrictions] (
    restrictions: List[Restriction[_, _]],
  ) {
  ...
  private def readOp[T](f: Restrictions => T): State[Restrictions, T] =
    State[Restrictions, T](s => (s, f(s)))
  ...
  def has(
    kind: RestrictionType,
    name: ResourceLocation,
  ): State[Restrictions, Boolean] = readOp(_.has(kind, name))
}
