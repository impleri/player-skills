...
import net.impleri.playerskills.api.skills.ChangeableSkill
...
case class SkillsDataLoader()
  ...
  ): Unit = CollectionConverters.asScala(data)
  ...
  private def parseSkill(name: ResourceLocation, json: JsonElement): Option[Skill[_]] = {
    ...
    val changesAllowed = parseInt(raw, "changesAllowed").getOrElse(ChangeableSkill.UNLIMITED_CHANGES)
    ...
    skillType
      ...
      .flatMap {
        case BasicSkillType.NAME => Some(BasicSkill(
        ...
        case NumericSkillType.NAME => Some(NumericSkill(
        ...
        case TieredSkillType.NAME => Some(TieredSkill(
          value = parseString(raw, "initialValue"),
          options = parseOptions(raw, _.getAsString),
          teamMode = parseTeamMode(raw, Some(TeamMode.Pyramid())),
          name = name,
          description = description,
          changesAllowed = changesAllowed,
          announceChange = notify,
        ...
        case SpecializedSkillType.NAME => Some(SpecializedSkill(
          value = parseString(raw, "initialValue"),
          options = parseOptions(raw, _.getAsString),
          teamMode = parseTeamMode(raw, Some(TeamMode.SplitEvenly())),
          name = name,
          description = description,
          changesAllowed = changesAllowed,
          announceChange = notify,
  ...
  private def createTeamMode(mode: String, rate: Option[Double] = None): Either[Exception, TeamMode] = mode match {
  ...
  private def parseNotify(
    ...
    parseString(raw, "notify") match {
      case Some(v) if v.nonEmpty => (true, Some(v))
