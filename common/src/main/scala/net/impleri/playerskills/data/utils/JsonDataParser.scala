...
trait JsonDataParser {
  ...
  ): Option[Boolean] = parseValue(raw, key, _.getAsBoolean, defaultValue)

  protected def parseInt(
    raw: JsonObject,
    key: String,
    defaultValue: Option[Int] = None,
  ): Option[Int] = parseValue(raw, key, _.getAsInt, defaultValue)

  protected def parseDouble(
    raw: JsonObject,
    key: String,
    defaultValue: Option[Double] = None,
  ): Option[Double] = parseValue(raw, key, _.getAsDouble, defaultValue)

  protected def parseString(
    raw: JsonObject,
    key: String,
    defaultValue: Option[String] = None,
  ): Option[String] = parseValue(raw, key, _.getAsString, defaultValue)
  ...
  protected def parseExclude[T](raw: JsonElement, callback: JsonElement => T): List[T] = parseArray(raw, "exclude",
    callback)

  protected def parseExcludeAction(raw: JsonElement, callback: JsonElement => Unit) = parseArrayEach(raw, "exclude",
    callback)

  protected def parseInclude[T](raw: JsonElement, callback: JsonElement => T): List[T] = parseArray(raw, "include",
    callback)

  protected def parseIncludeAction(raw: JsonElement, callback: JsonElement => Unit) = parseArrayEach(raw, "include",
    callback)
