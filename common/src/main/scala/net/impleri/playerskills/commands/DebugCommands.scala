...
trait DebugCommands {
      ...
      Commands.literal("debug")
        ...
        .`then`(
          Commands.literal("blocks")
            .executes(c => toggleDebug("Block Restrictions", c.getSource, () => PlayerSkillsLogger.BLOCKS.toggleDebug()))
        )
        .`then`(
          Commands.literal("fluids")
            .executes(c => toggleDebug("Fluid Restrictions", c.getSource, () => PlayerSkillsLogger.FLUIDS.toggleDebug()))
        )
        .`then`(
  ...
  private def toggleDebug(modLabel: String, source: CommandSourceStack, supplier: () => Boolean): Int = {
    val message = if (supplier()) Component.translatable("commands.playerskills.debug_enabled", modLabel).withStyle(ChatFormatting.RED, ChatFormatting.BOLD) else Component.translatable("commands.playerskills.debug_disabled", modLabel).withStyle(ChatFormatting.GREEN, ChatFormatting.ITALIC)
