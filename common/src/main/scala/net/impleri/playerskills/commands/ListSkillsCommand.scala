...
trait ListSkillsCommand extends ValuesCommandUtils {
  ...
  private def listSkills(source: CommandSourceStack): Int = {
    ...
    val message = if (skills.nonEmpty) Component.translatable("commands.playerskills.registered_skills", skills.size) else Component.translatable("commands.playerskills.no_registered_skills")
