...
object RegistrationTarget {
  final case object Namespace extends RegistrationTarget

  final case object Tag extends RegistrationTarget

  final case object Single extends RegistrationTarget
}

sealed abstract class RegistrationType[T](target: T) {
  def when(targetType: RegistrationTarget)(consumer: T => Unit): Unit = {
    target match {
...
object RegistrationType {
  ...
  def apply[T](value: String, registryKey: ResourceKey[Registry[T]]): Option[RegistrationType[_]] = {
    value.trim match {
      case s"@$namespace" => Option(RegistrationType.Namespace(namespace))
      case s"$namespace:*" => Option(RegistrationType.Namespace(namespace))
