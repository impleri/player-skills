...
interface SkillTaskTypes {
  companion object {
    ...
    val BASIC_SKILL: TaskType = TaskTypes.register(
      of("basic_skill_task"),
      { BasicSkillTask(it) },
      { Icon.getIcon("minecraft:item/wooden_hoe") },
    )

    val NUMERIC_SKILL: TaskType = TaskTypes.register(
      of("numeric_skill_task"),
      { NumericSkillTask(it) },
      { Icon.getIcon("minecraft:item/iron_hoe") },
    )

    val TIERED_SKILL: TaskType = TaskTypes.register(
      of("tiered_skill_task"),
      { TieredSkillTask(it) },
      { Icon.getIcon("minecraft:item/golden_hoe") },
    )

    val SPECIALIZED_SKILL: TaskType = TaskTypes.register(
